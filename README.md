...
- Windows에서는 `rpm2cpio`/`cpio`가 없을 때 `C:\Program Files\7-Zip\7z.exe`를 우선 사용해 rpm/cpio를 자동 추출합니다.
- 7-Zip이 없으면 WSL(Ubuntu)에 `rpm2cpio`, `cpio`가 설치된 경우 자동으로 WSL 명령을 사용해 추출을 진행합니다.
